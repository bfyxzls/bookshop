...
import com.lind.bookshop.exception.ErrorItem;
import com.lind.bookshop.exception.ErrorResponse;
...
public class ResponseUtils {
  ...
  public static ResponseEntity<?> ok(Object body, long totalRecords) {
    return ResponseEntity.ok(buildResponseBody(200, "操作成功", body, totalRecords));
  }
  ...
  public static ResponseEntity<?> ok(Object body, String message, long totalRecords) {
    return ResponseEntity.ok(buildResponseBody(200, message, body, totalRecords));
  }
  ...
  private static ResponseBody buildResponseBody(int statusCode, String message, Object body) {
    return ResponseBody.builder().status(statusCode).message(message)
        .data(body == null ? new HashMap<>() : body).build();
  }

  private static ResponseBody buildResponseBody(
      int statusCode, String message, Object body, long totalRecords) {
    return ResponseBody.builder().status(statusCode).message(message).totalRecords(totalRecords)
        .data(body == null ? new HashMap<>() : body).build();
  }
