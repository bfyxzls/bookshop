...
public class CommonUtils {
  ...
  public static String getRandomString() {
    ...
    logger.info("getRandomString:{}", number);
    return number;
  }
   ...
   * @param <T>
   */
  public static <T extends SortNumber> void sortNumber(List<T> sortNumbers) {
    int index = 1;
    sortNumbers = sortNumbers.stream()
        .sorted(Comparator.comparing(SortNumber::getSortValue).reversed())
        .collect(Collectors.toList());
    for (SortNumber item : sortNumbers) {
      item.setSortNumber(index);
      index++;
    }
  }
}
